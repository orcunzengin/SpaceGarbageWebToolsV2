window.iconCache = {}; 
        ...
        // GamePlanner modülleri de aynı debounce fonksiyonunu kullanabilsin
        ...
        // ==========================================
        // SIDEBAR BÖLÜM AÇ/KAPA (SpaceGarbage & GamePlanner grupları için ortak)
        // ==========================================
        window.toggleNavSection = function(key) {
            ...
            try { localStorage.setItem('navsection_' + key, body.classList.contains('collapsed') ? '1' : '0'); } catch(e) {}
        };
        // Sayfa açılışında daha önce kapatılmış bölüm varsa hatırla
        (function restoreNavSectionState() {
            ['sg', 'gp'].forEach(key => {
                try {
                    const collapsed = localStorage.getItem('navsection_' + key) === '1';
                    if (collapsed) {
                        const body = document.getElementById('body-' + key);
                        const arrow = document.getElementById('arrow-' + key);
        ...
        window.cancelConnectMode = function() {
            ...
            const b1 = document.getElementById('btn-pre1'); if(b1) b1.classList.remove('active-link');
            const b2 = document.getElementById('btn-pre2'); if(b2) b2.classList.remove('active-link');
            const wrapper = document.getElementById('canvas-wrapper');
        ...
        function openTool(toolId) {
            ...
            const panel = document.getElementById('connect-ui-panel'); if(panel) panel.style.display = 'none';
